...
for a=1:length(ag)
    for b=1:length(bg)
        ...
        OS(2,N) = bg(b);
        OS(3:5,N) = [0 0 0]';
        [temp,exitflag] = optim_eval(X,OS(1:2,N),OS,ref,round(length(ref)*options(4)));
        OS(:,N) = temp';
        N = N + 1;
        if options(1)
            if (exitflag==1)
...
for a=1:length(starts)
    ...
    [temp] = optim_eval(X,OS(1:2,N),OS,ref,round(length(ref)*options(4)));
    OS(:,N) = temp';
    ps = [ps(1) N 0];
    N = N + 1;
    ...
    while pt
        ...
        % potential fix for impending OS index out of range error
        if N == size(OS,2)
          pt = 0;
        end
