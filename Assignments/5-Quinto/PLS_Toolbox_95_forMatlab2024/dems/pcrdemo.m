echo on
% PCRDEMO Demo of the PCR function
...
pause
%-------------------------------------------------
% See "pcr help" to get the valid settings for these options.
%
...
% structure that can be used directly by the PCR function. Preprocessing
% can be performed explicitly at the command line, but this allows the
% PCR function to perform preprocessing automatically during calibration
% and application to new data. The standard preprocessing structure will
% become a part of the standard model structure output by PCR.
...
model = pcr(xblock1,yblock1,6,options)
...
%
% pred  = pcr(xblock2,model,options);
...
valid = pcr(xblock2,yblock2,model,options)
