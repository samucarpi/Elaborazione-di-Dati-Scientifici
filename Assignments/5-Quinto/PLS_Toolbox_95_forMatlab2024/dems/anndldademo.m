echo on
%ANNDLDADEMO Demo of the ANNDLDA function
 
echo off
%Copyright Eigenvector Research, Inc. 2021 Licensee shall not recompile,
...
pause
%-------------------------------------------------
%ANNDLDA (ANN Deep Learning Discriminate Analysis) generates an ANNDL model which can be used
%to classify samples as belonging to one of at least two classes. Building
%a ANNDLDA model requires samples from at least two classes. ANNDLDA attempts
%to identify characteristic latent variables which can be used to segregate
%the samples into a single given class. 
 
pause
%---------------------------------- 
% Here the use of anndlda will be demonstrated on the archeological dataset
% "arch"...
...
pause
%----------------------------------
% To perform ANNDL-DA, we must provide a y-variable which identifies the
...
% all four classes. This requires that we simply pass the ANNDLDA function
...
pause
%----------------------------------
% The ANNDLDA function is called with the x and y data, plus any options, 
% to build the model. An ANNDLDA model having a single hidden layer with 3 
...
switch options.algorithm
  ...
  case 'tensorflow'
    options.tf.activation = 'tanh';
    options.tf.hidden_layer{1} = struct('type','Dense','units',100);
    nhid1 = options.tf.hidden_layer{1}.units;
end
rng(1);
 
model = anndlda(arch, y, options);
 
pause
%---------------------------------- 
% The "predicted y-values" of the ANNDLDA model are actually values around and
...
% sample is considered to be a member of the class. The ANNDLDA function
...
% calculated by ANNDLDA and stored in the model.classification.probability 
% field. (For more information on the ANNDLDA prediction probabilities see
% PLSDTHRES)
...
model = model.crossvalidate(arch, {'vet' 6 1}, nhid1);
