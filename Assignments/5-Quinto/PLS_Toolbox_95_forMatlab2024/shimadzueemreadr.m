function out = shimadzueemreadr(file, options)
%SHIMADZUEEMREADR Reads Shimadzu EEM files with Rayleigh removal.
%  Reads SHIMADZU EEM CSV files.
%
%  INPUT:
...
%
%I/O: out = shimadzueemreadr(file,options);
%
...
if nargin==0 | isempty(file)
  [file,pathname] = evriuigetfile({'*.csv','Shimadzu EEM Files (*.csv)'; ...
...
if (nargin<2)
  options = shimadzueemreadr('options');
end
options = reconopts(options,'shimadzueemreadr');

nfiles = length(file);
if nfiles>1 & strcmp(options.waitbar,'on')
  wh = waitbar(0,'Loading Shimadzu EEM Files (Close to Cancel)');
  options.textreadr.waitbar = 'off';
else
  wh = [];
  options.textreadr.waitbar = options.waitbar;
end
options.textreadr.useimporttool = 'no';
options.textreadr.autodelim = false;
axisscale = {};
axisscalename = {};
out = cell(nfiles,1);
for (i=1:nfiles)
  try
    temp = textreadr(file{i},options.textreadr);
  catch
  ...
  if (i==1)
    axisscalename{3} = 'Emission (nm)';
    axisscalename{2} = 'Excitation (nm)';
  end
  ...
  x = permute(x, [1 3 2]);
  x.name = file{i};
  x.author = 'Created By SHIMADZUEEMREADR';
...
if (nfiles==1)
  ...
  if (combine) % Combine DSOs
    ...
    temp.name = 'Multiple Shimadzu EEM Files';
    out = temp;
