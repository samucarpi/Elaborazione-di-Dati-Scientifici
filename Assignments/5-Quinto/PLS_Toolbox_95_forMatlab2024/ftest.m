...
case 1
  ...
  ic   = 1;     %itmax = 100;   %iteration count, max it count 3/02
  itmax = 1000;               %iteration count, max it count increased 9/09
  %xtol = 1e-10;  ftol = 1e-8;  %tolerance on x,  tol on function betainc(x)-p  3/02
  xtol = 1e-12;  ftol = 1e-10;  %tolerance on x,  tol on function betainc(x)-p  3/09
  xl   = 0.0;                 %x left
  xr   = 1.0;                 %x right
  fxl  = -p;                  %betainc(0)-p
  ...
  if ic>=itmax
    warning('EVRI:Ftest',['FTEST failed to converge. Number of iterations at maximum ',int2str(itmax)])
  end
...
case 2
  v = 2*d./(2*d+2*n*p);
  v = max(min(v,1),0);
  fstat = betainc(v,d,n);  %03/02 changed / to ./ to allow for vectors
otherwise
