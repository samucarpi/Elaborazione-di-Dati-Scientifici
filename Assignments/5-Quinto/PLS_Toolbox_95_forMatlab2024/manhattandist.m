function dist = manhattandist(x,basis,options)
%MANHATTANDIST Calculate Manhattan Distance betweem rows of a matrix.
% Calculates the Manhattan Distance (sum of the absolute value differences) from
...
%
%I/O: dist = manhattandist(x,options)
%I/O: dist = manhattandist(x,basis,options)
...
%BK based on euclideandist.m 2016
...
if ischar(x)
  ...
  options.diag = 0;  %diagonal == self sample distance (i.e., 0)
...
% get sizes of x & basis (& hard delete excluded rows & columns)
[m,n]   = size(x);
[bm,bn] = size(basis);% <- redundant line?
if ~isdataset(x)
  x = dataset(x);
end  
incl  = x.include;
...
for ii = 1:length(ivect)
  ...
  switch usemode
    case 1
      ...
      for j = 1:i-1
        dist(i,j) = single(sum(abs(x(i,:)-x(j,:)))); %
        dist(j,i) = dist(i,j);
      end
    ...
    case 11
      ...
      for jj = 1:length(subrange)
        ...
        dist(i,jj) = single(sum(abs(x(i,:)-x(subrange(jj),:)))); %
      end
    ...
    case 2
      ...
      colsum = 0;
      for v = 1:n
        colsum = colsum + abs(x(:,v)-x(i,v)); %
      end
      dist(:,ii) = single(colsum); %
      dist(i,ii) = options.diag;
      
    case 3
      %compare x to a basis
      for j = 1:bm
        dist(i,j) = single(sum(abs(x(i,:)-basis(j,:)))); %
      end
  end
