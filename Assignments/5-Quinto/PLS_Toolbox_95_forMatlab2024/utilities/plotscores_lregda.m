function a = plotscores_lregda(modl,test,options)
%PLOTSCORES_LREGDA Plotscores helper function used to extract info from model.
...
isLregda = strcmpi(modl.modeltype,'lregda') ;
a            = [];
alabel       = cell(0);
...
%   end
  ...
  if isLregda & isfield(modl.detail,'originalinclude') & ~isempty(modl.detail.originalinclude) & ~isempty(modl.detail.originalinclude{1})
...
%     end
  ...
  %Comment out these next two if statement b/c LREGDA does not plot cv
  %results
