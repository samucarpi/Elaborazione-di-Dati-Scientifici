...
%ETABLE/SUBSREF Retrieve fields of ETABLE objects.
...
if length(index)>1;
  ...
  if ~strcmp(index(1).type,'.')
    ...
    switch feyld
      ...
      case 'rowcount'
        mytbl = obj.table;
        val = mytbl.getTable.getRowCount;
      case 'columncount'
        mytbl = obj.table;
        val = mytbl.getTable.getColumnCount;
      case 'copy_all'
        val = copy_callback(obj,[],obj.parent_figure,obj.tag,'all');
      case 'copy'
        val = copy_callback(obj,[],obj.parent_figure,obj.tag,'data');
      case 'getscrollpane'
        jt = obj.java_table;
        val = evrijavaobjectedt(jt.getParent.getParent);
      case 'getdata'
        val = getdata(obj);
      otherwise
        val = obj.(feyld);
