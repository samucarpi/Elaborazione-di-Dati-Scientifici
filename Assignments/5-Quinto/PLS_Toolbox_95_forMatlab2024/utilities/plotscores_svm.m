function a = plotscores_svm(modl,test,options)
...
if strcmpi(modl.modeltype,'svmda')
  isSvmda = strcmpi(modl.modeltype,'svmda') ;
end
...
if isSvmda
  nclass = length(modl.detail.svm.model.label);
  ny     = 1;
else  %SVMR
  %ny = size(modl.pred{2},2); %number of Y variables WILL BE 1 for SVMR
  ny     = 1;
  nclass = 1;
end
...
if isempty(test) | options.sct==1
  ...
  if isSvmda
    ...
    if ~nores & (isSvmda | (length(modl.detail.res)>1 & ~isempty(modl.detail.res{2})));
      for ii=1:ny;
  ...
  if isSvmda & isfield(modl.detail,'originalinclude') & ~isempty(modl.detail.originalinclude) & ~isempty(modl.detail.originalinclude{1})
...
if ~isempty(test) %X-block must be present
  ...
  if ~nomeasured
    ...
    elseif options.sct
      ytest = ones(test.datasource{1}.size(1), ny)*nan; % svm/svmda only support one y var
    else
      ytest = [];  %no cal data and no y-block use empty (to skip loop below)
    end
    if ~isempty(ytest) & ~isSvmda
      for ii=1:ny
        b = [b, ytest(:,ii)];
