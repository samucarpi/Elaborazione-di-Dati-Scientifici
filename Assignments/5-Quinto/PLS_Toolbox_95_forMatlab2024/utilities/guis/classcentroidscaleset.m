function varargout = classcentroidscaleset(varargin)
%CLASSCENTROIDSCALESET GUI used to modfiy settings of of CLASSCENTROID.
...
%
%I/O:   p = classcentroidscaleset(p)
%I/O:   p = classcentroidscaleset('default')
%I/O:   p = classcentroidscaleset('defaultscale')
%
...
pp.calibrate = { '[data,out{1},out{2}] = classcentroid(data,userdata);' };
pp.apply     = { 'data = classcentroid(data,out{1},out{2});'   };
pp.undo      = { 'data = rescale(data,out{1},out{2});'  };
pp.out       = {};
pp.settingsgui   = 'classcentroidscaleset';
pp.settingsonadd = 0;
pp.usesdataset   = 1;
pp.caloutputs    = 2;
