function varargout = glogset(varargin)
%GLOGSET GUI used to modfiy settings of GLOG.
...
set(handles.lambda,'string',sprintf(numformat,p.userdata.lambda));
...
p.userdata.lambda = str2num(get(handles.lambda,'string'));

p.description = sprintf(['%s (lambda = ' numformat ')'], p.description, p.userdata.lambda);
...
p.description   = 'GLog';
p.calibrate     = {'data = glog(data,userdata.lambda);' };
p.apply         = {'data = glog(data,userdata.lambda);' };
p.undo          = {'data = glog(data,userdata.lambda,struct(''inverse'',1););' };
p.out           = {};
p.settingsgui   = 'glogset';
p.settingsonadd = 1;
p.usesdataset   = 1;
p.caloutputs    = 0;
p.keyword       = 'glog';
p.tooltip       = 'Generalized log transformation';
p.category      = 'Transformations';

if isempty(p.userdata) | ~isstruct(p.userdata) | ~isfield(p.userdata,'lambda')
  p.userdata.lambda    = 1e-9;          %defaults
end


% --------------------------------------------------------------------
function lambda_Callback(h, eventdata, handles, varargin)

val = str2num(get(handles.lambda,'string'));
if isempty(val); val = 0; end
set(handles.lambda,'string',sprintf(numformat,val));

%-------------------------------------------------------------------
function out = numformat
