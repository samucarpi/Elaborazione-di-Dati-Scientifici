function varargout = poissonset(varargin)
%POISSONSET GUI used to modfiy settings of POISSONSCALE.
...
%
%I/O:   p = poissonset(p)
% (or)  p = poissonset('default')
%
%See also: DETREND, POISSONSCALE, PREPROCESS
...
% 6/23/03 JMS changed caloutputs to zero (=no cal required for apply)
...
val = num2str(p.userdata.offset);
if isempty(val)
  val = '3';
end
set(handles.offset,'string',val,'value',str2double(val));
...
p.userdata = struct('offset',get(handles.offset,'value'));
...
p.description = sprintf('%s (scale offset = %#.3g%s)', p.description, p.userdata.offset, '%');
...
p.description = 'Poisson (Sqrt Mean) Scaling';
p.calibrate = { '[data,out{1}] = poissonscale(data,userdata);' };
p.apply = { 'data = poissonscale(data,out{1});' };
p.undo = { 'data = rescale(data,out{1}*0,out{1});' };
p.out = {};
p.settingsgui = 'poissonset';
p.settingsonadd = 0;
p.usesdataset = 1;
p.caloutputs = 1;
p.keyword = 'sqmnsc';
p.tooltip = 'Scale each variable by the square root of its mean with offset';
p.category = 'Scaling and Centering';

if ~isfieldcheck(p,'p.userdata.offset') | isempty(p.userdata.offset)
  opts = poissonscale('options');
  p.userdata.offset = opts.offset;          %default
end
...
val = str2double(get(handles.offset,'string'));
if ~isempty(val) & val<0 | val>100
  val = [];  %invalid? reset back to original value
end
if isempty(val)
  val = get(handles.offset,'value');
end
set(handles.offset,'string',num2str(val),'value',val);
