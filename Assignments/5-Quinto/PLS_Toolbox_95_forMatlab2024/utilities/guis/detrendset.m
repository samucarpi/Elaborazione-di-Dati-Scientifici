function varargout = detrendset(varargin)
...
%
%I/O:   p = detrendset(p)
% (or)  p = detrendset('default')
%
%See also: DETREND, PREPROCESS
...
set(handles.order,'value',p.userdata.order+1);
...
p.userdata.order = get(handles.order,'value')-1;
...
if p.userdata.order>3;
  ordinal = sprintf('%ith order',p.userdata.order);
else
  ordinal = {'offset' 'linear' '2nd order' '3rd order'};
  ordinal = ordinal{p.userdata.order+1};
end
  
p.description = sprintf('Detrend (%s)',ordinal);
...
p.description   = 'Detrend';
p.calibrate     = {'opts=baseline(''options''); opts.order=userdata.order; [data,out{2}]=baseline(data,opts);'};
p.apply         = {'opts=baseline(''options''); opts.order=userdata.order; [data,out{2}]=baseline(data,opts);'};
p.undo          = {'data=baseline(data,out{2});'};
p.out           = {};
p.settingsgui   = 'detrendset';
p.settingsonadd = 0;
p.usesdataset   = 1;
p.caloutputs    = 0;
p.tooltip       = 'Remove mean offset from each sample (row)';
p.category      = 'Filtering';

if isempty(p.userdata);
  p.userdata.order    = 1;          %defaults
end

if isnumeric(p.userdata);
  %convert to structure if not
  p.userdata = struct('order',p.userdata(1));  %defaults if passed as userdata (backwards compatibility)
end

% --------------------------------------------------------------------
function  order_Callback(h, eventdata, handles, varargin)
