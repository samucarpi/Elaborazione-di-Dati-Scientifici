function varargout = npreprocenterset(varargin)
...
%
%I/O:   p = npreprocenterset(p)
% (or)  p = npreprocenterset('default')
%
%See also: MNCN, PREPROCESS
...
set(handles.mode,'string',num2str(find(p.userdata{1}(1,:))),'value',find(p.userdata{1}(1,:)));  
...
temp(1,get(handles.mode,'value')) = 1;
...
p.out = {};
p.settingsgui = 'npreprocenterset';
p.settingsonadd = 1;
p.usesdataset = 1;
p.caloutputs = 1;
p.keyword = 'Centering';
p.tooltip       = 'Mean centering for multiway arrays.';
