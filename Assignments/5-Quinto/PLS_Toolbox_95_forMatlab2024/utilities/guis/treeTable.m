...
% $Revision: 1.4 $  $Date: 2013/08/06 14:31:16 $
  ...
  %try
      ...
      try
          ...
          jtable.setSelectionBackground(java.awt.Color(0.85, 0.85, .9));  % light-blue
          jtable.setSelectionForeground(java.awt.Color(0, 0, 0));
...
function tableChangedCallback(hModel,hEvent,jtable)
    ...
    try
      %Try to run custom data change callback.
      hh = get(jtable);
      if isfield(hh,'DataChangeCallbackCustom')
        feval(hh.DataChangeCallbackCustom{:},hModel,hEvent,jtable)
      end
    end
