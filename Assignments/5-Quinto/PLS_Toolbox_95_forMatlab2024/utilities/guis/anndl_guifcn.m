function varargout = anndl_guifcn(varargin);
%ANNDL_GUIFCN ANNDL Analysis-specific methods for Analysis GUI.
...
anndlmode = getappdata(handles.analysis,'curanal');
opts = analysis('getoptshistory',handles,anndlmode);
if isempty(opts)
...
set(handles.tableheader,'string','ANNDL Model Statistics','HorizontalAlignment','center')

%Add view selections to dropdown.
panelinfo.name = [upper(anndlmode) ' Settings'];
panelinfo.file = 'anndlgui';
...
if strcmp(statmodl,'none') | strcmp(statmodl,'calnew');
    ...
    if ~strcmpi('anndlda', anndlmode) & ~isempty(y) & all(all(y.data(y.includ{1},:) == y.data(y.includ{1}(1),1)));
  ...
  try
    ...
    switch anndlmode
      ...
      case 'anndlda'
        if isempty(y)
          ...
          modl      = anndlda(x,y,opts);
        end
    end
    
    %get options structure that pertains to algorithm
    if strcmpi(modl.detail.options.algorithm,'sklearn')
      hidden_layers = modl.detail.options.sk.hidden_layer_sizes;
    elseif strcmpi(modl.detail.options.algorithm,'tensorflow')
      hidden_layers = modl.detail.options.tf.hidden_layer;
    end
    ...
    erdlgpls({'Error using ANNDL',lasterr},'Calibrate Error');
    return
  end

  % Do cross-validation (Currently, only do CV if it is BPN)
  % Warn that CV only done over layer 1 nodes
  if ~strcmp(cvmode,'none')  & length(hidden_layers) > 1
    evritip('anndl2layercv','ANNDL CV Tip: CV only tests over Layer 1 nodes. Other layer nodes not tested', [1])
  end
  if ~strcmp(cvmode,'none') & ismember(lower(opts.algorithm),{'sklearn' 'tensorflow'})
...
if analysis('isloaded','validation_xblock',handles)
  ...
  try
    ...
    switch anndlmode
      case 'anndl'
        if isempty(y)
          test = anndl(x,modl,opts);
        else
          test = anndl(x,y,modl,opts);
        end
      case 'anndlda'
        if isempty(y)
          test = anndlda(x,modl,opts);
        else
          test = anndlda(x,y,modl,opts);
        end
    end
  catch
...
%SSQ Table not used for ANNDL or ANNDLDA. Somehow it looks like it gets created
%in some legacy code so code below works. It can be enabled by uncommenting
%in the future. Return for now. NOTE: This code was taken from ann_guifcn
%so it may need some adjustments. 
...
if isempty(opts) | ~strcmp(opts.functionname, anndlmode)
  switch anndlmode
    case 'anndl'
      opts = anndl('options');
    case 'anndlda'
