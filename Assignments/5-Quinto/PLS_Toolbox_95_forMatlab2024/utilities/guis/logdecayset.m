function varargout = logdecayset(varargin)
...
set(handles.decayrate,'string',num2str(p.userdata));
...
p.userdata = str2num(get(handles.decayrate,'string'));

p.description = sprintf('%s (scale = %#.3g)', p.description, p.userdata);
...
p.description   = 'Log Decay Scaling';
p.calibrate     = {'[data,out{1}] = logdecay(data,userdata);'};
p.apply         = {'data = scale(data,out{1}.*0,out{1});'};
p.undo          = {'data = rescale(data,out{1}.*0,out{1});'};
p.out           = {};
p.settingsgui   = 'logdecayset';
p.settingsonadd = 1;
p.usesdataset   = 1;
p.caloutputs    = 1;
p.tooltip       = 'Mean centers and variance scales a matrix using the log decay of the variable axis.';
p.keyword       = 'logdecay';
p.category      = 'Scaling and Centering';

if isempty(p.userdata);
  p.userdata    = 0.33;          %defaults
end


% --------------------------------------------------------------------
function decayrate_Callback(h, eventdata, handles, varargin)

val = str2num(get(handles.decayrate,'string'));
if isempty(val); val = 0; end
set(handles.decayrate,'string',num2str(val));
