function a = plotscores_ann(modl,test,options)
...
isAnnda = strcmpi(modl.modeltype,'annda') ;
a            = [];
alabel       = cell(0);
...
if isempty(test) | options.sct==1
  ...
  if isAnnda
    ...
    if ~nores & (isAnnda | (length(modl.detail.res)>1 & ~isempty(modl.detail.res{2})));
      for ii=1:ny;
  ...
  nhid1 = modl.detail.options.nhid1;   
  ...
  if isAnnda & isfield(modl.detail,'originalinclude') & ~isempty(modl.detail.originalinclude) & ~isempty(modl.detail.originalinclude{1})
