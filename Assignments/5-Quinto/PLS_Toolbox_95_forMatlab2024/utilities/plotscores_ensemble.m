function a = plotscores_ensemble(modl,test,options)
%PLOTSCORES_ENSEMBLE Plotscores helper function used to extract info from model.
...
if strcmpi(modl.modeltype,'ensembleda')
  isEnsembleda = strcmpi(modl.modeltype,'ensembleda') ;
end
...
if isEnsembleda
%   nclass = length(modl.detail.xgb.model.label);
    nclass = length(modl.classification.classids);
  ny     = 1;
else  %XGBR
  ny = size(modl.pred{2},2); %number of Y variables WILL BE 1 for XGBR
  %ny     = 1;
  nclass = 1;
end
...
for ii=1:nclass
  ...
  if isEnsembleda
      ...
      labelsuffix{ii} = sprintf('%i (%s)',modl.classification.classnums(ii),modl.classification.classids{ii});
      
%     classset = modl.detail.options.classset;
%     if prod(modl.datasource{2}.size)==0 & ~isempty(modl.detail.class{1,1,classset});
%       classstr = getclassstr(modl.detail.classlookup{1,1,classset},modl.detail.svm.model.label(ii));
%       labelsuffix{ii} = sprintf('%i (%s)',modl.detail.svm.model.label(ii),classstr);
%     else
%       labelsuffix{ii} = sprintf('%i',modl.detail.svm.model.label(ii));
%     end
  else
...
if isempty(test) | options.sct==1
  ...
  if ~nomeasured & length(modl.detail.data)>1 & ~isempty(modl.detail.data{2});
    for ii=1:ny;
      meas = modl.detail.data{2}.data(:,modl.detail.includ{2,2}(ii)); %%modl.detail.svm.model.label(ii));
      if ~isEnsembleda
        a = [a, meas];
  ...
  if isEnsembleda
    ...
    if ~nores & (isEnsembleda | (length(modl.detail.res)>1 & ~isempty(modl.detail.res{2})));
      for ii=1:ny;
  ...
  if ~nocvpred & isfield(modl.detail,'cvpred') & ~isempty(modl.detail.cvpred);
    for ii=1:ny;
      ...
      if isEnsembleda
        alabel{end+1} = ['Class CV Predicted'];
      else
  ...
  if ~nocvpred & ~nomeasured & ~nores
    for ii=1:ny;
      ...
      if isEnsembleda
        alabel{end+1} = ['Class CV Residual'];
      else
...
%   if isfieldcheck(modl,'modl.detail.svm.outlier.flag')
%     outlier = modl.detail.svm.outlier.flag;
%     if ~isempty(outlier)
%       a = [a, outlier~=1];
%       alabel{end+1} = ['Outlier Flag'];
%     else
%       nooutlierflag = 1;
%     end
%   else
...
%   end
  ...
  if isEnsembleda & isfield(modl.detail,'originalinclude') & ~isempty(modl.detail.originalinclude) & ~isempty(modl.detail.originalinclude{1})
...
if ~isempty(test) %X-block must be present
  ...
  if ~nomeasured
    ...
    elseif options.sct
      ytest = ones(test.datasource{1}.size(1), ny)*nan; % XGB/XGBda only support one y var
    else
      ytest = [];  %no cal data and no y-block use empty (to skip loop below)
    end
    if ~isempty(ytest) & ~isEnsembleda
      for ii=1:ny
        b = [b, ytest(:,ii)];
...
%   if ~nooutlierflag & isfieldcheck(test,'test.detail.svm.outlier.flag')
%     outlier = test.detail.svm.outlier.flag;
%     if isempty(outlier)
%       outlier = zeros(size(b,1),1)*nan;
%     else
%       outlier = (outlier~=1);
%     end
%     b = [b, outlier];
%     blabel{end+1} = ['Outlier Flag'];
%   end
