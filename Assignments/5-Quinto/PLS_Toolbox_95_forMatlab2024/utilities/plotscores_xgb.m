function a = plotscores_xgb(modl,test,options)
%PLOTSCORES_XGB Plotscores helper function used to extract info from model.
...
if strcmpi(modl.modeltype,'xgbda')
  isXgbda = strcmpi(modl.modeltype,'xgbda') ;
end
...
if isempty(test) | options.sct==1
  ...
  if isXgbda
    ...
    if ~nores & (isXgbda | (length(modl.detail.res)>1 & ~isempty(modl.detail.res{2})));
      for ii=1:ny;
...
%   end
  ...
  if isXgbda & isfield(modl.detail,'originalinclude') & ~isempty(modl.detail.originalinclude) & ~isempty(modl.detail.originalinclude{1})
