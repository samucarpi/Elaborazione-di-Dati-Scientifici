function model = parafacfor_pf2(varargin)
...
xsize = size(x);
if length(loads)==(order+1) % Only one sample
    order = order+1;
    xsize = [xsize 1];
end

% ADDITIONAL SETTINGS
Show = 1000; % How often are fit values shown
pf2opt = 0; % Dummy when pf2 is not fitted


constrain_implications = constraintsoptionsoverview(constraints,0,order);
[fit,rss,tssx] = residualssq(loads,x,order);
...
while ~flags
    ...
    % DO ALS
    [loads,fit,diagnostics,constraints] = alsstep(loads,x,fit,constraints,xsize);
    ...
    [x,fit,loads,oldloads,lineparam]= moddata(x,options,loads,oldloads,order,it,lineparam,modeltype,xsize);

    % CHECK CONVERGENCE
    oldrelchange = relchange;
    [flags,endtxt,constraints,pf2opt,relchange,abschange,isconverged] = ...
        convergencecheck(it,fit,oldfit,options,constraints,pf2opt,modeltype,loads,Show,flags,oldrelchange);
    switch mod(it,50)
        case 0
...
%---------------------------------------------------------------------
function [ssq,ressq,tssqw,xest] = residualssq(loads,x,order)
...
%---------------------------------------------------------------------
function [options,plots,alllae,constraints] = optionscheck(options,modeltype,standardoptions,order)
...
%---------------------------------------------------------------------
function [xmod,fit,loads,oldloads,lineparam]= moddata(xmod,options,loads,oldloads,order,it,lineparam,modeltype,xsize)

% GET FIT
[fit,out1,out2,xest] = residualssq(loads,xmod,order);
%Iterative preproc
...
if (it/2 == round(it/2) && options.line == 1 && it>5)   % Every fifth iteration do a line search if ls == 1
    ...
    if ~strcmpi(modeltype,'parafac2') % Linesearch does not work for pf2 but it is implicitly done inside the pf models run in pf2
        ...
        %         if lll==1
        [loads,lineparam] = linesrch(xmod,loads,oldloads,lineparam);
        %         else
...
%---------------------------------------------------------------------
function [flags,endtxt,constraints,pf2opt,relchange,abschange,isconverged] = convergencecheck(it,fit,oldfit,options,constraints,pf2opt,modeltype,loads,Show,flags,oldrelchange)
endtxt = ' ';
isconverged=0;
...
if it > 3
    ...
    elseif abschange < options.stopcrit(2)
        ...
        isconverged=0;
    elseif it>= options.stopcriteria.iterations
        flags = 1;
        endtxt = ' Maximum iterations hit. Hence, the algorithm did not converge';
        isconverged=0;
    elseif isnan(fit)
        flags = 1;
...
%---------------------------------------------------------------------
function [dA,lineparam] = linesrch(x,A,Ao,lineparam)
...
%---------------------------------------------------------------------
function [dA,DeltaMin] = linesrch2(x,A,Ao,Delta);
...
if strcmpi(modeltype,'parafac')||strcmpi(modeltype,'parafac2')
    ...
    elseif length(size(x)) == 3&& ~any(isnan(x(:)))&&~any(isinf(x(:))) &&~sum(xsize<nocomp)>1 && ~any(xsize==1)
...
for i=1:length(options)
    ...
    if strcmp(options{i}.type,'unconstrained')|strcmp(options{i}.type,'nonnegativity')|strcmp(options{i}.type,'unimodality')|strcmp(options{i}.type,'orthogonality')|strcmp(options{i}.type,'L1')|strcmp(options{i}.type,'l1 penalty')
...
defs = {
    ...
    'scaletype.value'             'Scale Type'    'select'        {'norm' 'max' 'area'}                'advanced'      'Normally, loading vectors are scaled to norm one and the variance is in the sample loadings/scores. Can be changed so that loadings are scaled to have maximum value one.';
