function varargout = crossval_lreg(varargin)
%CROSSVAL_LREG Helper function for LREG cross-validation.
...
out = reconopts(options.rmoptions,'lreg',0);
...
switch algorithm
  case {'none', 'lasso', 'ridge', 'elastic net'}
    opts.cvi = []; % CV not needed here
    model = lreg(calx,caly,opts.lambda,opts);
    
    %apply model to test data
    pred = lreg(tstx,model,opts);
    ypred = pred.pred{2};
