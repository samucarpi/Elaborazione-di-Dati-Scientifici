function varargout = crossval_anndl(varargin)
...
out = reconopts(options.rmoptions,'anndl',0);
...
switch opts.algorithm
  case 'sklearn'
    opts.sk.hidden_layer_sizes{1} = myncomp;
  case 'tensorflow'
    if ~strcmpi(opts.tf.hidden_layer{1}.type, 'Dropout')
      %okay to change the hidden 1 units except for dropout (needs float)
      opts.tf.hidden_layer{1}.units = myncomp;
    end
end
...
opts.compression = 'none';  % don't compress again
model = anndl(calx,caly,opts);
%apply model to test data
pred = anndl(tstx,model,opts);
ypred = pred.pred{2};
