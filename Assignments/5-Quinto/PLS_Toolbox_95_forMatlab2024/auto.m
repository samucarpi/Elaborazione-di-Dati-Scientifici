...
% Autoscaling subtracts the mean of each column (i.e. variable) of an input
% matrix and then divides each column by its standard deviation. This
% provides a matrix with columns of a mean of zero and a standard deviation
% of one, well-tempered for standard eigen analyses.
%
% Typically, the assumption of autoscaling is that the most significant
% contribution to each column is signal of interest (rather than noise).
% If the primary contribution to a column is noise, straight autoscaling
% will often amplify noise and decrease performance of subsequent models.
% See the various options for handling low standard deviation columns.
%
%  INPUT:
...
%I/O: auto demo
%
%See also: GSCALE, GSCALER, MEDCN, MNCN, NORMALIZ, NPREPROCESS, REGCON, RESCALE, SCALE, SNV
...
if ischar(varargin{1});
  ...
  if nargout==0; evriio(mfilename,varargin{1},options); else; ax = evriio(mfilename,varargin{1},options); end
  return;
end
...
if isa(x,'double')
  ...
  if isfinite(options.offset)
    ...
    if any(badstdx);
      if nargout>3 | strcmp(options.display,'on')
        for loop = find(badstdx);
          msg{end+1} = ['Column ' num2str(incl{2}(loop)) ' has a standard deviation of zero.'];
          if strcmp(options.display,'on')
            disp(msg{end});
          end
        end
...
if isdataset(originaldso);
  ...
  if anyexcluded(originaldso)
    ...
    originaldso.data(incl{:}) = ax;  %insert scaled data (included columns only)
  else
    originaldso.data = ax;
  end
  ax = originaldso;
