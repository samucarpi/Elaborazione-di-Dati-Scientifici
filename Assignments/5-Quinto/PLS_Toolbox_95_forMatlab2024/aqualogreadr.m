function out = aqualogreadr(file,options)
...
%
%I/O: out = aqualogreadr(file,options);
%
%See also: AREADR, DATASET, FLUCUT, JASCOEEMREADR, SPCREADR, TEXTREADR, XLSREADR

% Copyright © Eigenvector Research, Inc. 1998
% Licensee shall not re-compile, translate or convert "M-files" contained
%  in PLS_Toolbox for use with any software other than MATLAB®, without
%  written permission from Eigenvector Research, Inc.
%Modified textreadr nbg 10/11

x = []; out=[];
if nargin==0 | isempty(file)
  [file,pathname] = evriuigetfile({'*.dat','HORIBA A-TEEM/Aqualog (*.DAT)'; ...
...
if nargin<2
  options = aqualogreadr('options');
end
options     = reconopts(options,'aqualogreadr');

nfiles = length(file);
if nfiles>1 & strcmp(options.waitbar,'on');
  wh = waitbar(0,'Loading Aqualog Files (Close to Cancel)');
...
for i1=1:nfiles
  ...
  axisscale{3}     = fliplr(temp.data(1,2:end));
  if i1==1
    axisscalename{2} = 'Emission (nm)';
    axisscalename{3} = 'Excitation (nm)';
  end
  data(1,:,:)           = fliplr(temp.data(2:end,2:end));
...
x.name = file{i1};
x.author = 'Created By AQUALOGREADR';
...
if (nfiles==1)
  ...
  for (i=2:nfiles)
    ...
    for j=2:(length(axisscale))% Check axisscales
      ...
      %if ~(length(out{1}.axisscale{j}) == length(out{i}.axisscale{j}))
      % replacing the if statement with the above will reproduce the
      % "incorrect" importing behavior as denoted in helpdesk ticket
      % #252915.
        combine = false;
        break;
      end
  ...
  if (combine) % Combine DSOs
    ...
    try
      ...
      temp.name = 'Multiple Aqualog Files';
      out = temp;
