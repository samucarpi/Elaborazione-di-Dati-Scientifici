function model = parafac2(varargin)
...
%  normal PARAFAC model (also called PARAFAC1). It handles slab-wise deviations
%  between components in one mode as long as the cross-product of the
%  components stay reasonably fixed. This can be utilized for modeling
%  chromatographic data with retention time shifts, modeling certain batch data
%  of varying length etc. See Kiers, ten Berge & Bro, Journal of Chemometrics,
%  1999, 13, 275-294, for details on the algorithm.
%
%  The convention for PARAFAC2 is that samples are in the last mode
%  and the shifting profiles are in the first mode. E.g. if the samples
%  are in mode 1, and the shifting mode is mode 2, the call to PARAFAC2
%  might be model = parafac2(permute(x,[2,3,1]),ncomp);
%
%  The first mode loadings in PARAFAC2 are estimated as P{k}*H where P{k}
%  is an orthogonal matrix specific to level k in the third mode and H a
%  small square matrix common to all samples. To obtain the actual loadings
%  do the following Ak = model.loads{1}.P{k}*model.loads{1}.H;
%
%  INPUTS:
%         x = the multi-way array to be decomposed
%             If all slabs have similar size, X is an array as e.g. in parafac.
%             If the slabs have different size X is a cell array. Then
%             X{1} = X1; X{2} = X2; etc. - note the curly brackets.
%             It is mode 1 of the X{i} that can shift and/or have different
%             dimensions.
%     ncomp = the number of components to estimate
%
%  OPTIONAL INPUTS:
%   initval = If a PARAFAC2 model is input, the data are fit to this model where
%             the loadings for the first mode (scores) are estimated found.
%           = If the loadings are input (e.g. model.loads) these are used
%             as starting values. Type PARAFAC2 INITVAL for more help
%   options = is a structure that used to enable constraints, weighted loss
%             function, govern plotting and display, and input stopping
%             criteria, etc. Type PARAFAC2 OPTIONS for more help
%
%  OUTPUT:
%     model = standard model structure (See MODELSTRUCT), or
%     pred  = a structure with PARAFAC2 predictions [i.e. the loadings
%             for the third mode (scores) are estimated].
%
%  This routine uses alternating least squares (ALS) in combination with
%  a line search every fifth iteration.
%
%I/O: model   = parafac2(x,ncomp);                 % identifies model with default options (calibration step)
%I/O: model   = parafac2(x,ncomp,options);         % identifies model (calibration step)
%I/O: model   = parafac2(x,initval,options);       % identifies model based on initial values in initval (calibration step)
%I/O: options = parafac2('options');               % returns a default options structure
%I/O: pred    = parafac2(xnew,model);              % find scores for new samples given old model
%I/O: parafac2 demo
%
%See also: DATAHAT, GRAM, MPCA, OUTERM, PARAFAC, TLD, TUCKER, UNFOLDM
...
standardtol = [1e-9 1e-12 10000 2*60*60];
constraint = cell(order,1);

% User gave following code to get options when using on parallel setup.
% Needed several tries to get options. Not sure if this was for parfor or
% parfeval. If running in parallel and getting errors form options/pslpref,
% try uncommenting code below. 
%
% % Modified part: Urban Wuensch
% count = 0;
% err_count = 0;
% maxcount = 100;%Dont' try for ever sk
...
%     end
%     pause(rand(1,1))
%     coptions = constrainfit('options');
%   catch MyErr
%     err_count = err_count + 1;
%     warning(matlabpref.mat error caught!!!!!!')
%   end
%   count = count + 1;
% end
...
standardoptions = struct('name','options','display','on','plots','final','waitbar','on','weights',[],'stopcriteria',[],'stopcrit',[0 0 0 0],...
    'init',0,'line',1,'iterative',iterative,'scaletype',scaletype,'blockdetails','standard','coreconsist','on','samplemode',3);
standardoptions.preprocessing = {[]};     %See Preprocess
standardoptions.validate.help = 'Load a model in .autochrome with one component less than the current, to evaluate the probability of the current model to be over-fitted';
standardoptions.validate.autochrome = [];
...
varargin{1}=x;
[x,nocomp,initval,options,oldmodel,xsize,order]            = checkinputstructure(modeltype,varargin{:});
options = reconopts(options,'parafac2');
...
pf2opt=pfinneroptions(order,constraints,predictmode);
...
while ~flags
    ...
    % DO ALS  
    % Update P
    unfoldloads = outerm(loads(1:end-1),1,1); % Exclude last mode
    Y = [];
    if xsize(end)~=1
        xmod2=shiftdim(xmod,order-1);
    else
        xmod2=xmod(:)';
    end
    for k = 1:xsize(end)
        xk       = reshape(xmod2(k,:),xsize(1),prod(xsize(2:order-1)));
        Qk       = xk*(unfoldloads*diag(loads{end}(k,:))*H');
        P{k}     = Qk*psqrt(Qk'*Qk);       %  [u,s,v]  = svd(Qk.');P{k}  = v(:,1:F)*u(:,1:F)';
        xkp = (xk'*P{k})';
        Y(k,:) = xkp(:)';
    end
    Y = reshape(Y,[xsize(order) nocomp xsize(2:order-1)]);
    Y = permute(Y,[2:order 1]);
    
    % Update H,B,C etc using PARAFAC-ALS
    smallloads{1} = H;
    for k=2:order
        smallloads{k} = loads{k};
    end
    
    % And the remaining PARAFAC2 parameters
    pfmodel = parafacfor_pf2(Y,smallloads,pf2opt);
    %pfmodel = parafac(Y,smallloads,pf2opt);
    H = pfmodel.loads{1};
    loads(2:end) = pfmodel.loads(2:end);
    loads{1}.H=H;
    loads{1}.P=P;  
    ...
    [flags,endtxt,constraints,pf2opt,relchange,abschange,timespent,isconverged] = convergencecheck(it,fit,oldfit,timespent0,options,constraints,pf2opt,modeltype,loads,iter_rew,iter_w_conv,DumpToScreen,Show,flags,hwait,oldrelchange);
    switch mod(it,125)
        case 0
            %every 125 iterations, do this
...
% Note that possible weights are not included in corcondia below
[Consistency,G,E] = corcondia(Y,smallloads,[],0);
...
if length(varargin)>1
    if ismodel(varargin{2})
      if strcmpi(varargin{2}.modeltype,modeltype)|strcmpi(varargin{2}.modeltype,'parafac2') % Then it's a model struct
...
if strcmpi(modeltype,'tucker')
    ...
    for i=1:order
        if prod(nocomp)/nocomp(i)<nocomp(i)&&nocomp(i)==max(nocomp(:));
            ...
            warning('EVRI:Parafac2NcompLimit',[' Components in mode ',num2str(i),' has been set to ',num2str(nocomp(i)),' as additional components will not add to the fit'])
        end
    end
    if any(nocomp(:)>xsize(:))
        m = find(nocomp(:)>xsize(:));
        if length(m)>1
            warning('EVRI:Parafac2NcompLimit',[' The number of components in modes ',num2str(m(:)'),' exceed the dimension of those modes'])
        else
            warning('EVRI:Parafac2NcompLimit',[' The number of components in mode ',num2str(m),' exceed the dimension of that mode'])
        end
    end
elseif strcmpi(modeltype,'parafac')||strcmpi(modeltype,'parafac2')
...
if N==2 % Two-way fitting is simple
    ...
    if mode_off
        ...
        [A(ind_1,1:F),diagnostics{1},constraints{1}]=constrainfit(A(ind_1,1:F),pinv(prod(AtA(:,:,2:N),3)),Aold,constraints{1});
...
predictmode = 0;
if ismodel(oldmodel)
  if strcmpi(oldmodel.modeltype,modeltype)|(strcmpi(oldmodel.modeltype,'parafac2')) % Then it's a model struct
    predictmode = 1;
  else
    warning('EVRI:Parafac2ModelMismatch',[' Input fitted model (',upper(oldmodel.modeltype),') is not the same type as the function called (',upper(modeltype),')'])
  end
end
if predictmode
...
% DO LINESEARCH
if (it/2 == round(it/2) && options.line == 1 && it>5)   % Every second iteration do a line search if ls == 1
    lineparam.it = it;
...
if sum(freetoscalemodes(2:end)==1)>1 % Else no scaling
    if ScaleMode(1)~=1
        ScaleMode = ScaleMode(1);
    else
        try
            ScaleMode = ScaleMode(2);
        catch
            ScaleMode = 2;
        end
    end
    for i=2:length(loads)
        if (i ~=ScaleMode) && freetoscalemodes(i)
            if strcmpi(options.scaletype.value,'norm')
                SS = sum(loads{i}.^2,1);
                SS(SS==0)=1;% If a zero value is encountered
                Scal = 1./sqrt(SS);
        ...
        if ~isempty(fxcol)
            for i=2:length(loads)
                if (i ~=ScaleMode)
                    ...
                    loads{i}(:,nonfxcol) = loads{i}(:,nonfxcol)*diag(Scal(nonfxcol)*TargetScal); 
                    % normalization leads to wrong model but 
                    % that's corrected in the next update of the next mode, and for the last mode 
                    % no normalization is performed, so that's ok, unless last mode is fixed.
...
%---------------------------------------------------------------------
function [flags,endtxt,constraints,pf2opt,relchange,abschange,timespent,isconverged] = convergencecheck(it,fit,oldfit,timespent0,options,constraints,pf2opt,modeltype,loads,iter_rew,iter_w_conv,DumpToScreen,Show,flags,hwait,oldrelchange)
endtxt = ' ';
isconverged=0;
...
if it > 1
    ...
    elseif abschange < options.stopcrit(2)
        flags = 1;
        endtxt=' Iterations terminated based on absolute change in fit error. Hence the algorithm did not converge. You may want to lower this criterion or simply multiply your array by a big constant';
        isconverged=0;
    elseif it > options.stopcrit(3)-1
        flags = 1;
        endtxt = ' Iterations terminated based on maximum iterations. Hence the algorithm did not converge. You may want to allow more iterations';
        isconverged=0;
    elseif timespent > options.stopcrit(4)
        flags = 1;
        endtxt = ' Iterations terminated based on maximum time. Hence the algorithm did not converge. You may want to allow more more time';
        isconverged=0;
    elseif isnan(fit)
        flags = 1;
        endtxt = ' Non-feasible numerical solution';
        isconverged=0;
    ...
    if flags==1
        if DumpToScreen
            ...
            disp(' ')
            if findstr(endtxt,'did not converge')
                warning('EVRI:Parafac2General',endtxt)
            else
                disp(endtxt)
            end
        end
    ...
    if relchange~=0&&strcmpi(options.waitbar,'on')
        if ishandle(hwait)
            if iter_rew
                ...
                g=waitbar((options.stopcrit(1)/((relchange+oldrelchange)/2)).^.3,hwait);
            end
        end
    end
end
...
if DoWeight
    if alllae
        xsq = abs((x-datahat3(param)).*weights);
    else
        xsq = ((x-datahat3(param)).*weights).^2;
    end
else
    if alllae
        xsq = abs(x-datahat3(param));
    else
        xsq = (x-datahat3(param)).^2;
    end
end
if Missing
    xsq(MissId)=0;
...
model = modelstruct('PARAFAC2',order);
model.date = date;
model.time = clock;
model.loads                = x0;
...
Xsdo2 = Xsdo;
inc = Xsdo2.include;
inc{options.samplemode} = 1:size(Xsdo2,options.samplemode);
Xsdo2 = Xsdo2(inc{:});
[xhat,rawres] = datahat3(x0,Xsdo2.data); % All samples
inc2 = Xsdo2.include;
if options.samplemode>length(inc2)
    % Can happen if there is only one sample in the last mode
else
    inc2{options.samplemode}=Xsdo.include{options.samplemode}; % To use for residuals a few lines down
end
xhat = datahat3(x0);
rawres = Xsdo2.data-xhat;
...
if ~strcmpi(options.blockdetails,'compact')
    if ~predictmode
        ...
        reslim95 = residuallimit(rawres(inc2{:}),.95,resopts);
        reslim99 = residuallimit(rawres(inc2{:}),.99,resopts);
        temp = [];
        temp.lim95 = reslim95;
        temp.lim99 = reslim99;
...
% Make a version of xhar where excluded samples are excluded
    ...
    xhat_small = outerm(x0_out(2:end-1),0,1);% Outer product of the inner PARAFAC part
    xhat = repmat(nan,prod(xsize),nocomp);
    PH = repmat(nan,xsize(1)*xsize(end),nocomp);
    for k=1:size(x0_out{end},1)
        PHk = x0_out{1}.P{k}*x0_out{1}.H;
        PHkScaled = PHk*diag(x0_out{end}(k,:));
        PH((k-1)*xsize(1)+1:k*xsize(1),:)=PHkScaled;
    end
    for i = 1:nocomp
        xhat(:,i) = vec(PH(:,i)*xhat_small(:,i)');
    end
...
if isa(options.validate.autochrome,'evrimodel')
    % Then apply autochrome assuming that the model attached in the options
    % is the model with one component less
        prob = autochromprob(Xsdo,model,options.validate.autochrome);
        model.detail.validation.autochrome.probability_of_overfit = prob;
end
...
if iscell(x0) || isstruct(x0)  % Old loadings given
...
elseif order==2 && size(x0,1)==xsize(1) % Then its mcr and input scores are given
    ...
    elseif initialization == 5
        warning('EVRI:Parafac2Compression','Compression not adviced for PARAFAC2 - using rational start instead')
        [x0,InitString] = rationalstart(x,nocomp,modeltype,options);
...
if strcmpi(modeltype,'parafac')||strcmpi(modeltype,'parafac2')
    ...
    elseif isstruct(x0)
        ...
        if ~(strcmpi(x0.modeltype,'parafac')||strcmpi(x0.modeltype,'parafac2'))
            ...
            x0 = x0.loads;
            %set scores to random values (the actual size may be different from
            %the old model scores)
            ...
            norm1 = norm(x0{sampmode})/xsize(sampmode);%norm1 = norm(x0{options.samplemode})/xsize(options.samplemode);
            norm2 = norm(initloads{sampmode})/size(initloads{sampmode},1);%norm2 = norm(initloads{options.samplemode})/size(initloads{options.samplemode},1);
            x0{sampmode} = (x0{sampmode}/norm1)*norm2;%x0{options.samplemode} = (x0{options.samplemode}/norm1)*norm2;
            InitString = [' Fitting to old model (finding scores in mode ', num2str(sampmode),')'];%InitString = [' Fitting to old model (finding scores in mode ', num2str(options.samplemode),')'];
            for i=1:length(x0)
                if i~=sampmode%if i~=options.samplemode
            ...
            constraints{sampmode}.fixed.weight = 0;  % Unfix any element in sample mode as they are to be found% constraints{options.samplemode}.fixed.weight = 0;  % Unfix any element in sample mode as they are to be found
        end
    elseif ~iscell(x0)
...
bestmodel = feval(lower(modeltype),x,nocomp,0,interimopt2);
...
for ccount = 2:initialization(1)
    ...
    currentmodel = feval(lower(modeltype),x,nocomp,interimopt);
...
if strcmpi(modeltype,'parafac')||strcmpi(modeltype,'parafac2')
    ...
    elseif length(size(x)) == 3&& ~any(isnan(x(:)))&&~any(isinf(x(:))) &&~all(xsize<nocomp) && ~any(xsize==1)
...
constrainedmodes     = 1;% Mode one is always fixed scale-wise in PF2
freetoscalemodes     = 0; % Mode one is always fixed scale-wise in PF2
fixmodes             = 0;
for i=2:length(options)
    if strcmp(options{i}.type,'unconstrained')
        constrainedmodes(i)=0;
    else
        constrainedmodes(i)=1;
    end
    if strcmp(options{i}.type,'unconstrained')|strcmp(options{i}.type,'nonnegativity')|strcmp(options{i}.type,'orthogonality')|strcmp(options{i}.type,'L1')
        freetoscalemodes(i)=1;
    else
        freetoscalemodes(i)=0;
    end
    if strcmp(options{i}.type,'columnwise') % Then maybe fixed
        for i2=1:length(options{i}.columnconstraints)
            if any([3 5 6 20]==options{i}.columnconstraints{i2}(1))
                fixmodes = 1;
            end
        end
    elseif strcmp(options{i}.type,'equality')|strcmp(options{i}.type,'rightprod')
        fixmodes = 1;
    end
            ...
            for jj = 1:length(options.constraints) % Modify constraints for each mode
                ...
                if strcmp(myconstr.functionname,'regresconstr') ...
                    ...
                    if myconstr.nonnegativity
                        ...
                        onew.nonnegativity.algorithmforglobalmodel = myconstr.nonnegativityalgorithm;
                        if verb
...
function pf2opt=pfinneroptions(order,constraints,predictmode)
pf2opt             = parafac('options');
pf2opt.display     = 'off';
pf2opt.stopcriteria.relativechange = 1e-10;
pf2opt.stopcriteria.absolutechange = 1e-10;
pf2opt.stopcriteria.iterations = 10;
pf2opt.plots       = 'off';
pf2opt.waitbar     = 'off';
%pf2opt.blockdetails='compact';
pf2opt.samplemode = order;
for i=2:order
    pf2opt.constraints{i} = constraints{i};
end
if predictmode
    pf2opt.constraints{1}.type = 'Dont change';
end
...
function [xhat,resids] = datahat(loads,data)

% Sped up version of datahat.m for internal use in PARAFAC2
...
mm = reshape(repmat(NaN,prod(xsize),1),[xsize(1),xsize(2),prod(xsize(3:end))]);
for k = 1:xsize(k);
    mm(k,:,:) = (P{k}*H)*diag(loads{end}(k,:))*m';
end
mm = permute(mm,[2:length(loads) 1]);
xhat = reshape(mm,xsize);
if nargin>1
    resids = data-xhat;
end

function [xhat,resids] = datahat3(loads,data)
...
mm = reshape(NaN(prod(xsize),1),[xsize(end),xsize(1),prod(xsize(2:end-1))]);
for k = 1:xsize(end);
    mm(k,:,:) = (P{k}*H)*diag(loads{end}(k,:))*m';
end
mm = permute(mm,[2:length(loads) 1]);
