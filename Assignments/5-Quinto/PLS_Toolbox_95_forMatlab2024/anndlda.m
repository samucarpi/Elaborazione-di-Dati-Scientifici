function varargout = anndlda(varargin)
%ANNDLDA Deep Learning Artificial Neural Net discriminant analysis.
% ANNDLDA is a artificial neural net discrimination method used
%  to classify samples. The y-block in a ANNDLDA model indicates which
...
%
%  The prediction from a ANNDLDA model is a value of nominally zero or one. A
...
%  This implementation uses third party software:
...
%                     ANNDLDA to use. Both have no limitations on the number
...
%                   [https://wiki.eigenvector.com/index.php?title=Anndlda#Cross-validation].
...
%                    applying the ANNDLDA model. 'pca' uses a simple PCA
%                    model to compress the information. 'pls' uses a pls
%                    model. Compression can make the ANNDLA more stable and
...
%               sk: Sub-structure for Python sklearn ANNDLDA parameters. Used
...
%               tf: Sub-structure for Python tensorflow ANNDLDA parameters.
...
%
%      Example: To change options for the sklearn ANNDLDA, set algorithm to
%      'sklearn' and modify the sk sub-structure:
%         >>>opts = anndlda('options')
%         >>>opts.sk.activation = 'tanh'
%
%      Example: To change options for the tensorflow ANNDLDA, set algorithm to
...
%
%I/O: model = anndlda(x,y,options);  %identifies model (calibration step)
%I/O: model = anndlda(x,options);  %identifies model (calibration step)
%I/O: pred  = anndlda(x,model,options);    %makes predictions with a new X-block
%I/O: valid = anndlda(x,y,model,options);  %makes predictions with new X- & sample classes
%I/O: options = anndlda('options');        %returns a default options structure
%I/O: anndlda demo
%
%See also: CLASS2LOGICAL, COMPRESSMODEL, CROSSVAL, DISCRIMPROB, KNN,
%MODELSELECTOR, PLS, PLSDAROC, PLSDTHRES, SIMCA, VIP ANNDA
...
elseif nargin==1 & ischar(varargin{1}) %Help, Demo, Options
  ...
  options.tf.loss       = 'binary_crossentropy';  %hard-code default categorical loss function
  ...
  error(['Error in parsing input arguments See help(''anndlda'') for more information on passing parameters.']);
end

%find options, reconcile and store for later
% We'll pass most options to ANNDL but some will be specially handled here
optsind = [];
modelind = [];
...
if isempty(optsind);
  ...
  varargin{end+1} = anndlda('options');
  optsind = length(varargin);
else
  %found options, reconcile with standard options
  varargin{optsind} = reconopts(varargin{optsind},'anndlda',{'outputversion' 'rawmodel'});
end
...
model = anndl(varargin{:});

%copy model over to ANNDLDA structure
if ~isempty(modelind)
  template = varargin{modelind};
else
  switch options.algorithm
    case 'sklearn'
      template = modelstruct('anndlda','sk');
    case 'tensorflow'
      template = modelstruct('anndlda','tf');
  end
end
...
if ~isempty(modelind)
  varargin{modelind}.modeltype = 'ANNDLDA';  %reset back to model type ANNDA
  if ~isfield(options,'rawmodel') | ~options.rawmodel
    model.modeltype = 'ANNDLDA_PRED';  %hard-code as ANNDA Prediction type model
  else
    model.modeltype = 'ANNDLDA';  %hard-code as ANNDA model (rawmodel call)
  end
else
  model.modeltype = 'ANNDLDA';  %hard-code as ANNDA type model
end
...
defs = {
  ...
  'algorithm'              'Standard'       'select'        {'sklearn' 'tensorflow'}         'novice'        [{'Algorithm to use. ''sklearn'' is the default. ''sklearn''/''tensorflow'' use Python implemntations. '} getanndldacp];
  'cvskip'                 'Standard'       'double'        'int(1:inf)'                     'novice'        'Control the step size in cross-validation over the number of nodes in the first hidden layer. Empty is the default, so it will incorporate a ''smart rule'' in determining what nodes to cross-validate over. More on this at [https://wiki.eigenvector.com/index.php?title=Anndlda#Cross-validation]. Otherwise, an integer can be passed to specify the increment between nodes to cross-validate over.'
  ...
  'tf.loss'                'Tensorflow'     'select'        {'binary_crossentropy' 'categorical_crossentropy' 'poisson'} 'novice' 'Choice of loss function to be minimized.'
  ...
  'tf.min_delta'           'Tensorflow'     'double'        'float(0:inf)'                   'novice'        'Minimum change in the loss to count as an improvement.'
...
function out = getanndldacp()

out  = {'This ANNDLDA method in PLS_Toolbox uses the Scikit-Learn framework, provided '...
  'by Scikit-Learn API, under the terms of the BSD 3-Clause license. ANNDLDA also uses the Tensorflow framework, provided '...
