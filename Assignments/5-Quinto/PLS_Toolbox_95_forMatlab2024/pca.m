...
%                       level is used by the robust algorithm to indicate
%                       which sample(s) are considered outside the limits
%                       and, therefore, likely outliers. It does NOT
%                       indicate which samples were actually left out (see
%                       alpha above), but only those samples which appear
%                       to be more unusual. Default value is the same value
%                       as confidencelimit (if non-zero) or alpha (if
%                       confidencelimit is zero.)
%
%  OUTPUT:
...
options = reconopts(varargin{3},mfilename,{'rawmodel' 'samplemode' 'classset' 'staticplots' 'rule' 'strictthreshold'});
...
if ~predictmode | options.outputversion ~= 3;
  ...
  if mdcheck(x);
    if strcmp(options.display,'on'); warning('EVRI:MissingDataFound','Missing Data Found - Replacing with "best guess". Results may be affected by this action. Samples and variables which cannot be fixed will be excluded.'); end
    [flag,missmap,x] = mdcheck(x);
  end
  
  model.detail.includ = x.include';      %copy all mode labels, etc.
  ...
  switch options.algorithm
    ...
    case 'robustpca'
      ...
      %determine what value to use for robust cutoff value
      if isempty(options.roptions.cutoff)
        %no cutoff value passed
        if options.confidencelimit>0;
          %use confidencelimit if non-zero
          cutoff = options.confidencelimit;
        else
          %use alpha-based cutoff if no confidence limit available
          cutoff = options.roptions.alpha;
        end
      else
        %actual cutoff value passed
        cutoff = options.roptions.cutoff;
      end
      if cutoff>1; cutoff = cutoff/100; end
      
      robpcaout = robpca(xpp.data(xpp.includ{1},xpp.includ{2}),'k',varargin{2},'kmax',varargin{2},'alpha',options.roptions.alpha,'plots',0,'cutoff',cutoff);
...
'blockdetails'           'Standard'       'select'        {'standard' 'all'}               'novice'        'Extent of predictions and raw residuals included in model. ''standard'' = none, ''all'' x-block.';
...
'roptions.cutoff'        'Robust Options' 'double'        'float(0:1)'                     'novice'        'Confidence cutoff level used by robust algorithm to indicate probable "outlier" (used only for display purposes) Default = same as confidencelimit. These options are only used when algorithm is robust PCA.';
};
