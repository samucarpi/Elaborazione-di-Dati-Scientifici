function varargout = lregda(varargin)
%LREGDA Logistic Regression discriminant analysis.
% LREGDA is a softmax multinomial logistic regression method for
% discriminant analysis applied to input X and Y blocks (or an X-block 
...
%  are never used when calibrating a LREGDA model. The model will include
...
%                    applying the LREG model. 'pca' uses a simple PCA
%                    model to compress the information. 'pls' uses a pls
%                    model. Compression can make the LREG more stable and
...
%
%  OUTPUT:
%     model = standard model structure containing the LREGDA model (See MODELSTRUCT).
%      pred = structure array with predictions
%     valid = structure array with predictions
%
%I/O: model = lregda(x,y,options);  %identifies model (calibration step)
%I/O: model = lregda(x,options);  %identifies model (calibration step)
%I/O: pred  = lregda(x,model,options);    %makes predictions with a new X-block
%I/O: valid = lregda(x,y,model,options);  %makes predictions with new X- & sample classes
%I/O: options = lregda('options');        %returns a default options structure
%I/O: lregda demo
%
%See also: LREG, CLASS2LOGICAL, COMPRESSMODEL, CROSSVAL
...
% We'll pass most options to LREG but some will be specially handled here
...
if isempty(optsind);
  ...
  varargin{end+1} = lregda('options');
  optsind = length(varargin);
else
  %found options, reconcile with standard options
  varargin{optsind} = reconopts(varargin{optsind},'lregda',{'outputversion' 'rawmodel'});
end
...
%copy model over to LREGDA structure
if ~isempty(modelind)
  template = varargin{modelind};
else
  template = evrimodel('lregda');
end
...
if ~isempty(modelind)
  varargin{modelind}.modeltype = 'LREGDA';  %reset back to model type LREGDA
  if ~isfield(options,'rawmodel') | ~options.rawmodel
    model.modeltype = 'LREGDA_PRED';  %hard-code as LREGDA Prediction type model
  else
    model.modeltype = 'LREGDA';  %hard-code as LREGDA model (rawmodel call)
  end
else
  model.modeltype = 'LREGDA';  %hard-code as LREGDA type model
end
end
...
'algorithm'              'Standard'    'select'        {'none' 'lasso' 'ridge' 'elastic net'}           'novice'   'LREG algorithm to use. "Ridge" is default. "None", "Lasso", "Elastic Net" are alternative choices.';
