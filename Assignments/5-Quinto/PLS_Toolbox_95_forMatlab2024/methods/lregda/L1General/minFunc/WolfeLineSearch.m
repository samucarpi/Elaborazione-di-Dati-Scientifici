...
while LSiter < maxLS
    ...
    elseif LS_interp == 2
        ...
        t = polyinterpMF([temp f_prev gtd_prev; t f_new gtd_new],doPlot,minStep,maxStep);
    elseif LS_interp == 3
...
while ~done && LSiter < maxLS
    ...
    elseif LS_interp == 2
        ...
        t = polyinterpMF([bracket(1) bracketFval(1) bracketGval(:,1)'*d
...
alpha_c = polyinterpMF([x0 f0 g0; x1 f1 g1],doPlot,minStep,maxStep);
alpha_s = polyinterpMF([x0 f0 g0; x1 sqrt(-1) g1],doPlot,minStep,maxStep);
...
if bracketFval(Tpos) > oldLOFval
    alpha_c = polyinterpMF([oldLOval oldLOFval oldLOgtd
        bracket(Tpos) bracketFval(Tpos) gtdT],doPlot);
    alpha_q = polyinterpMF([oldLOval oldLOFval oldLOgtd
...
elseif gtdT'*oldLOgtd < 0
    alpha_c = polyinterpMF([oldLOval oldLOFval oldLOgtd
        bracket(Tpos) bracketFval(Tpos) gtdT],doPlot);
    alpha_s = polyinterpMF([oldLOval oldLOFval oldLOgtd
...
elseif abs(gtdT) <= abs(oldLOgtd)
    ...
    alpha_s = polyinterpMF([oldLOval sqrt(-1) oldLOgtd
        bracket(Tpos) bracketFval(Tpos) gtdT],...
        doPlot,min(bracket),max(bracket));
    ...
    t = polyinterpMF([bracket(nonTpos) bracketFval(nonTpos) gtdNonT
        bracket(Tpos) bracketFval(Tpos) gtdT],doPlot);
end
end