...
while f_new > fr + c1*t*gtd || ~isLegal(f_new)
    ...
    elseif LS_interp == 1 || ~isLegal(g_new)
        ...
        if funEvals < 2 || LS_multi == 0 || ~isLegal(f_prev)
            ...
            t = polyinterpMF([0 f gtd; t f_new sqrt(-1)],doPlot,0,t);
        else
            % Backtracking w/ cubic interpolation based on three points
            if debug
                fprintf('Cubic BT\n');
            end
            t = polyinterpMF([0 f gtd; t f_new sqrt(-1); t_prev f_prev sqrt(-1)],doPlot,0,t);
        end
    else
        ...
        if funEvals < 2 || LS_multi == 0 || ~isLegal(f_prev)
            ...
            t = polyinterpMF([0 f gtd; t f_new g_new'*d],doPlot,0,t);
        elseif ~isLegal(g_prev)
            ...
            t = polyinterpMF([0 f gtd; t f_new g_new'*d; t_prev f_prev sqrt(-1)],doPlot,0,t);
        else
            ...
            t = polyinterpMF([0 f gtd; t f_new g_new'*d; t_prev f_prev g_prev'*d],doPlot,0,t);
         end
    end
