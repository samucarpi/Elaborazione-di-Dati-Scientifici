function [issues,splitinfo] = reviewhmaccrossval(varargin)
...
% Inputs:
%    model : a standard model structure or the handle to the CV GUI.
%
% Output:
...
try
  if ~ismodel(modl)
    if ~isa(modl,'hmacCvGui'); return; end
    %if ~strcmpi(get(analh,'tag'),'analysis'); return; end
    cvh     = modl.fig; %getappdata(analh,'crossvalgui');
