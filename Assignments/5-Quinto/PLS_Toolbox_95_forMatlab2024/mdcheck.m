...
%                  will operate, and
%        toomuch: [ 'error' | {'exclude'} | 'ask' ] What action should be
%                  taken if too much  missing data is found. 'error' exit
%                  with error message, 'exclude' will exclude elements
%                  which contain too much missing data before replacement.
%                  'ask' will prompt the user for whether to replace or
%                  cancel the operation (throwing an error.)
%      algorithm: [ {'svd'} | 'nipals' | 'knn' ] specifies missing data
%                  replacement algorithm to use. NIPALS typically used for
%                  large amounts of missing data or large multi-way arrays.
%                  KNN works for sparsely populated data sets
%         cursor: [ 'off' | {'on'} ] Show crosshair cursor with data position 
%                  reported at edge.
%
...
%
%See also: EXCLUDEMISSING, PARAFAC, PCA, REPLACE
...
if ischar(data);
  ...
  options.toomuch = 'exclude'; % 'error' | 'exclude'
...
%we did NOT force to be a dataset, consider it as-is
wasdso_beforetoomuch = isa(data,'dataset');

%if asked to, exclude elements with too much missing data
if nargout>2
  switch options.toomuch
    case {'ask' 'exclude'}
      %exclude the data which is missing too many elements
      [data,bad] = excludemissing(data,options.max_missing);
      if ~all(cellfun('isempty',bad))
        %found some bad data
        if strcmpi(options.toomuch,'ask')
          ask = questdlg('Some samples/variables contained too much missing data to replace. Exclude these items or Cancel operation?','Too Much Missing','Exclude','Cancel','Exclude');
          if strcmpi(ask,'Cancel')
            error('Some samples or variables contained too much missing data. Change MDCHECK > MAX_MISSING option to allow replacement.')
          end
        end
        if ~strcmpi(options.display,'off')
          warning('EVRI:TooMuchMissing','Some data was excluded and ignored due to too much missing data');
        end
      end
  end
end
...
if wasdso
  ...
  incl = data.include;
  data = data.data(incl{:});
  anyexcld = anyexcluded(origdata); %any(size(data)~=size(origdata));
  if anyexcld & (nargout>1 | strcmp(options.plots,'final'))  %NOTE: Must match logic at end which looks for origflag
    [origflag,origmissmap] = mdcheck(origdata.data);  %store missing map of ALL data
  end
else
  anyexcld = false;
  origflag = false;
...
if ~isempty(data);
  ...
  %empty data is (by definition) not missing
  flag = false;
end

if ~flag
  if nargout>1 | strcmp(options.plots,'final')
    missmap = false(size(data));
  end
else
  missmap = ~isfinite(data);
  flag    = sum(missmap);
  while length(flag)>1; flag = sum(flag); end;
  flag    = flag./numel(data);           %calculate fraction of missing data
        ...
        while pcs < options.max_pcs & frac_ssq < options.frac_ssq
          ...
          while change > options.tolerance(1)
            ...
            switch lower(options.algorithm)
              case 'nipals'
                x = data;
                x(missmap==1) = NaN; % Put back NaNs (should be changed to not infilling them at all when nipals is chosen)
              ...
              case {'svd' 'knn'}
                ...
                switch lower(options.algorithm)
                  ...
                  case 'knn'
                    [pclass,closest,votes]=knn(data,1,struct('display','off'));
                    mddd = data(closest,:);
                end
            ...
            estdata = data;
            estdata(missmap==1) = mddd(missmap==1);
            
            if strcmpi(options.recalcmean,'yes') & strcmpi(options.meancenter,'yes')
...
%if was DSO, adjust missmap size and replaced data to expected format
if wasdso & (nargout>1 | strcmp(options.plots,'final'))
  if anyexcld
    %if any were excluded
    if ~origflag & ~flag
      %shortcut fast method to create all false missing map (nothing
      %missing in anything - included or excluded)
      missmap = false(size(origmissmap));
    else
      %something missing in either included or excluded, insert into
      %original missingmap (with all samples)
      missmap = nassign(origmissmap,missmap,incl,1:length(incl));
    end
    if nargout>2
      %asked for data
      if ~flag
        %nothing missing in included data, return original data
        if wasdso_beforetoomuch
          %pass back DSO
          data = origdata;
        else
          %pass back raw
        ...
        %something was missing, so we've done a replacement. insert back
        %into full data
        temp = nassign(origdata.data,data,incl,1:length(incl));
        if wasdso_beforetoomuch
          %pass back DSO
          origdata.data = temp;
          data = origdata;
    ...
    %nothing excluded
    %don't need to do anything to the missing map but...
    if nargout>2
      %asked for data?
      if wasdso_beforetoomuch
        %pass back DSO
