...
%  INPUTS:
%    x    = m by n matrix of observations (measurements),
%    stdx = m by n matrix of standard deviations associated
%           with the observations in x.  For missing measurements,
%           stdx should be set to zero.
%    p    = dimensionality of the subspace sought
...
%                   XML=U*S*V'
%     SOBJ = value of the objective function for the best model.
...
%    - U*S gives the maximum likelihood scores (which can be
%      used for PCR
%
...
%      equation gives the ML estimate in the original space:
%               xml = V*t
%
...
% 
%  Relevant references:
%   P.D. Wentzell and M.T. Lohnes, "Maximum Likelihood Principal Component
%     Analysis with Correlated Measurement Errors Theoretical and Practical
%     Considerations", Chemom. Intell. Lab. Syst., 45, 65-85 (1999).
%   P.D. Wentzell, D.T. Andrews, D.C. Hamilton, K. Faber, and B.R. Kowalski,
%     "Maximum likelihood principal component analysis", J. Chemometrics,
%      11(4), 339-366 (1997).
%   P.D. Wentzell, D.T. Andrews, and B.R. Kowalski, "Maximum 
%     likelihood multivariate calibration", 69, 2299-2311 (1997).
%   D.T. Andrews and P.D. Wentzell, "Applications of maximum  likelihood
%     principal components analysis: Incomplete data and calibration transfer",
%     Anal. Chim. Acta, 350, 341-352 (1997).
%   For further information, contact: Peter D. Wentzell
%                                     Dept. of Chemistry, Dalhousie University
%                                     Halifax, NS, B3H 4J3,  Canada 
%                                     Ph. 902-494-3708 or 3305
%                                     Fax 902-494-1310
%                                     e-mail:wentzell@chem3.chem.dal.ca
%
%I/O: [U,S,V,SOBJ,ErrFlag] = mlpca(x,stdx,p);
%
%See also: ANALYSIS, MCR, PARAFAC, PCA
...
% Copyright Peter D. Wentzell, 1997-2006

if nargin == 0; X = 'io'; end
varargin{1} = X;
if ischar(varargin{1});
  options = [];
  if nargout==0; evriio(mfilename,varargin{1},options); else; U = evriio(mfilename,varargin{1},options); end
  return; 
end
