...
if rmsecv 
    ...
    wait_handle = waitbar(0,'Please wait...','numbertitle','off','name','Crossvalidation');
        
    for i = 1:n
        waitbar(i/n,wait_handle)
...
if j ~= kmax
    for noCsteps = 1:10
        ...
        [sortresid,indsr] = sort(residu);
        obj = sum(sortresid(1:(h),:).^2);
        [Q,R] = qr([Tk_min_i(indsr(1:(h)),:) ones((h),1)],0);
        z = R\(Q'*y_min_i(indsr(1:(h)),1));
        sloperaw = z(1:j,:);
        intraw = z(j+1,:);
        if noCsteps >= 10 | abs(obj - prevobj) < 10^(-4)
           if noCsteps >= 10
