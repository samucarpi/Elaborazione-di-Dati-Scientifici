...
%                See makeplot.m for more information
...
% Last Update: 17/06/2003,  03/07/2006
...
default=struct('alpha',alfa,'h',h,'labsd',labsd,'labod',labod,...
    'k',k,'plots',plots,'kmax',kmax,'mcd',mcd,'classic',0,'scree',scree,'press',press,'robpcamcd',robpcamcd,'cutoff',cutoff);
    ...
    switch dummy
        case 0 % Take on default values
            ...
            if any(strcmp(chklist,'kmax'))
                ...
                kmax = options.kmax;
                options.h=min(floor(2*floor((n+kmax+1)/2)-n+2*(n-floor((n+kmax+1)/2))*alfa),n);
            else
                options.h=h;
            end
        case 3
...
if p1<=min(floor(n/5),kmax) && mcd
    ...
    if h~=n
        ...
        true = (ndirect == nrich1);
        B=extradir(T1,ndirect,seed,true); %n*ri
        for i=1:size(B,1)
            Bnorm(i)=norm(B(i,:),2);
        end
    ...
    if (scree == 1)||(press == 1)
        ...
        % and h can change again, because they are now based on the kmax
        % value.
        if dummy == 2
    ...
    if ~robpcamcd
        center=clm;
        out.P = P1*P2(:,1:k);
        out.M = center + clmX*P1';
        result=struct('P',{out.P},'M',{out.M},'k',{k},'h',{h},'Hsubsets',{out.Hsubsets});
        return
    end
    ...
    outprMCD = projectMCD(Tn,L,k,h,niter,rot,P1,P2,center,cutoff);
    ...
    out.h = h;
    out.alpha = alfa;
    out.Hsubsets.H1 = outprMCD.Hsubsets.H1;
    out.Hsubsets.Hfreq = outprMCD.Hsubsets.Hfreq;
end
...
if options.classic == 1
    outDist = CompDist(data,r,outprMCD,cutoff,out.classic);
else
    outDist = CompDist(data,r,outprMCD,cutoff);
end
...
result=struct('P',{out.P},'L',{out.L},'M',{out.M},'T',{out.T},'k',{out.k},'kmax',{kmax},'alpha',{out.alpha},'h',{out.h},'Hsubsets',{out.Hsubsets},...
...
% these function performs the last part of ROBPCA when k is determined.
% input :
...
outprMCD = out;
%--------------------------------------------------------------------------------
function outDist = CompDist(data,r,out,cutoff,classic)
...
if options.classic==1
    ...
    out.classic.flag=((out.classic.od<=out.classic.cutoff.od)&(out.classic.sd<=out.classic.cutoff.sd));
...
if k~=r
    out.flag=(out.od<=out.cutoff.od)&(out.sd<=out.cutoff.sd);
else
    out.flag=(out.sd<=out.cutoff.sd);
end

outDist = out;

%----------------------------------------------------------------------------
function B2=extradir(data,ndirect,seed,true)
...
for ndir=1:ndirect
    ...
    B2(ndir,:)=data(index(1),:)-data(index(2),:);
end
