...
%                labeled on the outlier map (default = 3)   
%      classic : If equal to one, the classical SIMPLS analysis will be performed
...
%                      (orthogonal outliers) and/or whose residual distance is larger than result.cutoff.rd
...
default=struct('alpha',alfa,'h',h,'labsd',labsd,'labod',labod,'labresd',labresd,'k',k,'kr',kr,...
    ...
    'kmaxr',kmaxr,'rmsep_value',rmsep_value,'rmsecv_value',rmsecv_value,'rsquared_value',rsquared_value,'rss_value',rss_value);
...
if nargin>2
    ...
    st=options.st;
    out=options.out;rmsecv=options.rmsecv;classic=options.classic;
    rmsep=options.rmsep;
...
elseif q1>1 & k>=((h/q1)-(q1/2)-0.5)
    mess=sprintf(['Attention (rsimpls.m): The number of components, k = ',num2str(k),...
            '\n is larger than our recommended maximum value of k = ',num2str(max(1,floor((h/q1)-(q1/2)-1.5))),...
            '\n (based on samples and Y-columns).']);
    disp(mess)
end
...
if k == 0
    if rmsecv == 0
        ...
        out=rrmse(x,y,h,kmax,'RSIMPLS',options.plots);
        if k==0 & ~plots
            result.rmsecv=out.rmsecv;
            return
        end
        R2 = out.R2;
        rss = out.rss;
    ...
    if isnan(rsquared_value)&isnan(rss_value)&isnan(rmsecv_value)
        out.rcs = 0;
    end
   ...
   out.cov=robpcareg.cov;
    ...
    if ~st
        ...
        %Score distance
        out.sd=sqrt(mahalanobis(out.t,robpcareg.center(1:k),'cov',robpcareg.sigma(1:k,1:k)))';
        out.cutoff.sd=sqrt(chi2inv(0.975,k));
        %robust residual distance
        if q1==1
            out.rd=out.res/sqrt(out.cov); 
        else
            out.rd=sqrt(mahalanobis(out.res,zeros(1,q1),'cov',out.cov))';
        end
        ...
        Yw=y(out.flag==1,:);
        cYw=mcenter(Yw);    
        res=out.res(out.flag==1,:);
        out.rsquared=1- sum(sum(res.^2))/sum(sum(cYw.^2));
        ...
        result=struct('slope',{out.slope}, 'int',{out.int},'fitted',{out.yhat},'res',{out.res}, 'cov',{out.cov},...
            'T',{out.T}, 'weights', {out.weights},  'rsquared',{out.rsquared},'rcs',{out.rcs},'rmsep',{out.rmsep},...
            'k',{out.k},'h',{out.h},'alpha',{out.alpha},'sd', {out.sd},'od',{out.od},...
            'resd',{out.rd},'cutoff',{out.cutoff},'flag',{out.flag},'class',{out.class},'classic',{resultclassic},'robpca',{out.robpca});
