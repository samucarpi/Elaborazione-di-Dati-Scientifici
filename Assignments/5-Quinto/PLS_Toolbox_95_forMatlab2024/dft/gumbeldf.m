function out = gumbeldf(method,x,a,b,options)
%GUMBELDF Gumbel distribution function.
%  INPUTS:
%     method = string defining type of request (see below).
%         x = depends on call (see below).
%         a = Mode parameter (real).
...
%
%I/O:  cumulative = gumbeldf('cumulative',x,a,b,options)
%            x is ordinate in range (-inf,inf)
%            f(x) = exp(-exp(-(x-a)./b))
%I/O:  density = gumbeldf('density',x,a,b,options)
%            x is ordinate in range (-inf,inf)
%            density is f(x) density distribution at x.
%            factor = (x-a)./b
%            f(x) = exp(-factor).*exp(-exp(-factor))./b
%I/O:  quantile = gumbeldf('quantile',u,a,b,options)
%            u is cumulative distribution at q in range (0,1)
%            quantile is Q ordinate.
%            f(x) = a - b.*real(log(-log(x)))
%I/O:  random = gumbeldf('random',n,a,b,options)
%            n is the size of the random matrix to generate.
%            random is a 'n' sized matrix of random numbers drawn from f(x).
%
%See also: BETADF, CAUCHYDF, CHIDF, EXPDF, GAMMADF, LAPLACEDF, LOGNORMDF, LOGISDF, NORMDF, PARETODF, RAYDF, TRIANGLEDF, UNIFDF, WEIBULLDF
...
switch lower(method)
  ...
  case 'density'
    ...
    b(find(b <= 0)) = NaN;
    factor = (x-a)./b;
    density = exp(-factor).*exp(-exp(-factor))./b;
    out = density;
