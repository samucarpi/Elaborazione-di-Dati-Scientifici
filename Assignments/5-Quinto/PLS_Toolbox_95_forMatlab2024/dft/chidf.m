function out = chidf(method,x,a,options)
%CHIDF Chi squared distribution function.
%  INPUTS:
%     method = string defining type of request (see below).
%         x = depends on call (see below).
%         a = degrees of freedom parameter (positive integer).
%
%         NOTE: Under certain conditions inputs will be resized by RESIZE
%         function. For more information type "resize help". 
%
%  OPTIONAL INPUT:
%   options = structure array with the following fields used:
%         name: 'options'.
%
%  OUTPUT:
%I/O:  prob = chidf('cumulative',x,a,options)
%            x is ordinate in range (0,inf)
%            prob is F(x) cumulative distribution at x.
%               F(x) =  gammainc(x/2,a/2)
%I/O:  density = chidf('density',x,a,options)
%            x is ordinate in range (0,inf)
%            density is f(x) density distribution at x.
%               f(x) =  (x.^(a/2-1) .* exp(-x))/(2 * gamma(a/2));
%I/O:  quantile = chidf('quantile',u,a,options)
%            u is cumulative distribution at q in range (0,1)
%            quantile is Q ordinate.
%               prob = q ordinate
%               Q(u) = prob
%I/O:  random = chidf('random',n,a,options)
%            n is the number of random numbers to generate
%            random is vector of random numbers drawn from f(x).
%  For more information type "chidf help".
%
%See also: BETADF, CAUCHYDF, EXPDF, GAMMADF, GUMBELDF, LAPLACEDF, LOGNORMDF, LOGISDF, NORMDF, PARETODF, RAYDF, TRIANGLEDF, UNIFDF, WEIBULLDF

%Copyright (c) Eigenvector Research, Inc. 2000
% Licensee shall not re-compile, translate or convert "M-files" contained
%  in PLS_Toolbox for use with any software other than MATLAB¨, without
%  written permission from Eigenvector Research, Inc.
%
%Acquired from TESS, Texas Environmental Software Solutions
% Version 1.0.0, January 1998
%nbg 04/03 - mod to PLS_Toolbox
%rsk 01/09/06 - update to PLS_Toolbox

if nargin==0; method = 'io'; end
if ismember(method,evriio([],'validtopics'));
  options = [];
  options.name     = 'options';
  if nargout==0; evriio(mfilename,method,options);
  else;          out = evriio(mfilename,method,options); end
  return;
end

%--Check Missing Inputs
if nargin<2
  error([mfilename,' requires at least 2 inputs.'])
end
if nargin<3
  a = 1;
end

%--Check Method
method  = ck_function(method);

%--Check Inputs
if ~isreal(x)
  error('Input x must be positive and real.');
end
if ~isreal(a)
  error('Input ''a'' must be a real.');
end

%--Run Method
switch lower(method)
  case 'cumulative'
    [x a] = resize(x,a);
    x(find(x < 0)) = NaN;
    a(find(a <= 0)) = NaN;
    cumulative = ensurep(gammainc((x)./2,a./2));
    out = cumulative;

  case 'density'
    [x a] = resize(x,a);
    x(find(x < 0)) = NaN;
    a(find(a <= 0)) = NaN;
    nu2 = a./2;
    density = ((x).^(nu2-1) .* exp(-(x)./2)) ./ (2.^nu2 .* gamma(nu2));
    out = density;

  case 'quantile'
    [x a] = resize(x,a);
    quantile = gammadf('inv',x,a./2,2);
    out = quantile;

  case 'random'
    if ~isint(x) | ~isnonneg(x)
      error('Input x must be integer > 0.');
    end
    if ~ispos(a)
      error('Input a must be positive.')
    end
    [dummyx,a] = resize(ones(x),a);
    x = size(dummyx);
    sza = size(a);
    %Set x to largest parameter (if didn't happen with resize).
    if ~isscal(a)
      if length(sza)~=length(x) | any(sza~=x)
        x = sza;
      end
    end
    random = gammadf('random',x,a./2,2);
    out = random;
    
end
