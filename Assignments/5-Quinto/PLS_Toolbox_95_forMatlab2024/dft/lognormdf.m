function out = lognormdf(method,x,a,b,options)
%LOGNORMDF LogNormal distribution function.
%  INPUTS:
...
%
%I/O:  cumulative = lognormdf('cumulative',x,a,b,options)
%            x is ordinate in range (-inf,inf)
%            f(x) = 0.5*(1 - erf((a-real(log(x))) ./ (sqrt(2)*b)))
%I/O:  density = lognormdf('density',x,a,b,options)
%            x is ordinate in range (-inf,inf)
%            density is f(x) density distribution at x.
%            f(x) = 1/sqrt(2*_pi)*exp(-0.5.*((real(log(x))-a)./b).^2) ./(b.*(x))
%I/O:  quantile = lognormdf('quantile',u,a,b,options)
%            u is cumulative distribution at q in range (0,1)
%            quantile is Q ordinate.
%            f(x) = exp(sqrt(2).*b .* erfinv(2.*x-1) + a);
%I/O:  random = lognormdf('random',n,a,b,options)
%            n is the size of the random matrix to generate.
%            random is a 'n' sized matrix of random numbers drawn from f(x).
%
%See also: BETADF, CAUCHYDF, CHIDF, EXPDF, GAMMADF, GUMBELDF, LAPLACEDF, LOGISDF, NORMDF, PARETODF, RAYDF, TRIANGLEDF, UNIFDF, WEIBULLDF
...
switch lower(method)
  case 'cumulative'
    ...
    x(find(x < 0)) = NaN;
    b(find(b <= 0)) = NaN;
    p = 0.5*(1 - erf((a-real(log(x))) ./ (sqrt(2)*b)));
  ...
  case 'density' 
    ...
    x(find(x <= 0)) = NaN;
    b(find(b < 0)) = NaN;
    factor = 0.398942280401;		% 1/sqrt(2*_pi)
    density = factor*exp(-0.5.*((real(log(x))-a)./b).^2) ./(b.*(x));
    out = density;
  ...
  case 'quantile'
    ...
    x(find(x < 0 | x > 1)) = NaN;
    b(find(b <= 0)) = NaN;
    quantile = exp(sqrt(2).*b .* erfinv(2.*x-1) + a);
    out = quantile;
  ...
  case 'random'
    ...
    r = rand(x);
    random = lognormdf('quantile',r,a,b);
    out = random;
