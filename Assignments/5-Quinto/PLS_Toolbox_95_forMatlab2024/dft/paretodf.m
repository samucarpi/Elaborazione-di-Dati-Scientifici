function out = paretodf(method,x,a,b,options)
%PARETODF Pareto distribution function.
%  INPUTS:
...
%
%I/O:  cumulative = paretodf('cumulative',x,a,b,options)
%            x is ordinate in range (-inf,inf)
%            f(x) = 1 - (a./x).^b
%I/O:  density = paretodf('density',x,a,b,options)
%            x is ordinate in range (-inf,inf)
%            density is f(x) density distribution at x.
%            f(x) = b .* (a./x).^b ./ x
%I/O:  quantile = paretodf('quantile',u,a,b,options)
%            u is cumulative distribution at q in range (-inf,inf)
%            quantile is Q ordinate.
%            f(x) = a .* (1-x) .^ (-1./b)
%I/O:  random = paretodf('random',n,a,b,options)
%            n is the size of the random matrix to generate.
%            random is a 'n' sized matrix of random numbers drawn from f(x).
%
%See also: BETADF, CAUCHYDF, CHIDF, EXPDF, GAMMADF, GUMBELDF, LAPLACEDF, LOGNORMDF, LOGISDF, NORMDF, RAYDF, TRIANGLEDF, UNIFDF, WEIBULLDF
...
switch lower(method)
  case 'cumulative'
    [x,a,b] = resize(x,a,b);
    x(find(x < a)) = NaN;
    a(find(a <= 0 | isnan(a))) = NaN;
    b(find(b <= 0 | isnan(b))) = NaN;
    p = 1 - (a./x).^b;
  ...
  case 'density'
    ...
    a(find(a < 0 | isnan(a))) = NaN;
    b(find(b < 0 | isnan(b))) = NaN;
    %TODO: check equation. Changed (b * (a./x)...) to (b .* (a./x)...)
    density = b .* (a./x).^b ./ x; 
    out = density;
